...
Driver::Driver(bool service) : Listener(), config_path(""), connected(false), service(service)
{
    load_config();
}

Driver::Driver(std::string config_path, bool service)
    : Listener(), config_path(config_path), connected(false), service(service)
{
    load_config();
}
...
void Driver::process()
{
    ...
    int finger_count = allTheFingers.count() - allTheFingers.fingerType(Leap::Finger::TYPE_THUMB).count();
...
void Driver::gesture(Leap::FingerList fingers)
{
    ...
    {
        int id = (*it).id() % 5;
        finger_postitons[id].r =
            sqrt(pow((x_center - (*it).tipPosition().x), 2) + pow((y_center - (*it).tipPosition().y), 2) +
                 pow((z_center - (*it).tipPosition().z), 2));

        finger_postitons[id].psi =
            atan2(sqrt(pow((x_center - (*it).tipPosition().x), 2) + pow((y_center - (*it).tipPosition().y), 2)),
                (z_center - (*it).tipPosition().z)) *
            360 / (2 * M_PI);

        finger_postitons[id].phi =
            atan2((y_center - (*it).tipPosition().y), (x_center - (*it).tipPosition().x)) * 360 / (2 * M_PI);
    }
    ...
    else
    {
        ...
        std::transform(finger_postitons.begin(),
            finger_postitons.end(),
            old_finger_postitons.begin(),
            d_finger_postitons.begin(),
            [](spherical s1, spherical s2) { return s2 - s1; });

        std::transform(d_finger_postitons.begin(),
            d_finger_postitons.end(),
            incr_finger_postitons.begin(),
            incr_finger_postitons.begin(),
            [](spherical s1, spherical s2) { return s1 + s2; });
...
void Driver::load_config()
{
    ...
    try
    {
        if (!file)
        {
            throw std::runtime_error(std::string("Can't open the config file \"") + config_path + std::string("\": ") +
